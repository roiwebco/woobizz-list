...
if (!function_exists( 'woobizz_main_menu' ) ) {
	...
	add_action( 'admin_menu', 'woobizz_main_menu',4 );
}
//WELCOME PAGE
if (!function_exists( 'woobizz_welcome_page' ) ) {
	function woobizz_welcome_page(){
		$woobizz_sidebar_welcome_title= __( 'Welcome to Woobizz!', 'woobizz-list' );
		$woobizz_sidebar_welcome_description= __( 'Woobizz give you the right tools to create great e-commerce sites with wordpress and woocommerce', 'woobizz-list' );
		$woobizz_sidebar_welcome_link= __( 'To find more please visit', 'woobizz-list' );
		?>
...
//ADD PLUGIN TITLE 
if (!function_exists( 'woobizz_show_list_plugin_title' ) ) {
	function woobizz_show_list_plugin_title() {
		if (strpos($_SERVER['REQUEST_URI'], "page=woobizz-list") !== false){
			echo"<style>#tab-list:before{content:'WooBizz List'!important;text-transform: uppercase;margin:18px 18px 18px 18px;display:inherit;</style>";
		}else{
		 //do nothing
		}
	}
	add_action ('in_admin_header','woobizz_show_list_plugin_title');
}